...
这就是说**，JavaScript 语言的底层根本没有整数，所有数字都是小数**（64位浮点数）。容易造成混淆的是，某些运算只有整数才能完成，此时 JavaScript 会自动把64位浮点数，转成32位整数，然后再进行运算，参见《运算符》一章的“[[docs/operators/bit|位运算]]”部分。

由于浮点数不是精确的值，**所以涉及小数的比较和运算要特别小心**。(Menma:应该有类似java bigdecimal计算相关的库吧？)
...
数组的`indexOf`方法内部使用的是[[docs/operators/comparison#严格相等运算符|严格相等运算符]]，所以该方法对`NaN`不成立。
...
如果字符串以`0x`或`0X`开头，`parseInt`会将其按照十六进制数解析。[[#数值的进制]]

```javascript
parseInt('0x10') // 16
```

如果**字符串以`0`开头，将其按照10进制解析**。（Menma:和八进制还不一样.. 输入八进制字符串好像没用？）
...
`isNaN`方法可以用来判断一个值是否为`NaN`。（Menma:判断这个感觉没啥意义，直接判断是不是number是不是足够了？）
