...
对象（object）是 JavaScript 语言的核心概念，也是**最重要的数据类型**。
...
对象的所有**键名**都是**字符串**（ES6 又引入了 Symbol 值也可以作为键名），所以加不加引号都可以。上面的代码也可以写成下面这样。
...
如果键名**不符合标识名的条件**（比如第一个字符为数字，或者含有空格或运算符），且也不是数字，则**必须加上引号**，否则会报错。
...
对象的每一个键名又称为“属性”（property），它的“键值”可以是任何数据类型。如果一个属性的值为**函数**，通常把这个属性称为“方法”，它可以像函数那样调用。
...
此时，**如果取消某一个变量对于原对象的引用，不会影响到另一个变量**。**（Menma：var o2 = o1; 将o2指向的o1的对象，再改变o1的引用并不会对原来的对象造成影响）**
...
但是，**这种引用只局限于对象**，如果两个变量指向同一个原始类型的值。那么，变量这时都是值的拷贝。**（Menma：原始类型的引用改变直接在内存中拷贝一份再指向，导致地址不是同一个）**
...
上面的代码中，当`x`的值发生变化后，`y`的值并不变，这就**表示`y`和`x`并不是指向同一个内存地址**。
...
请注意，如果使用方括号运算符，**键名必须放在引号里面（除了数字）**，否则会被当作变量处理。
...
上面代码之中，对象`obj`的`p`属性是不能删除的，所以`delete`命令返回`false`（关于`Object.defineProperty`方法的介绍，请看《标准库》[[note/stdlib/object|object]]的 Object 对象一章）。
